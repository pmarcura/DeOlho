...
function FonteBadge({ source }: { source: "pncp" | "portal" }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded ring-1", cfg.cls)}>
    ...
    <Card>
      <CardContent className="p-5 sm:p-6">
        <div className="flex items-center justify-between gap-2 mb-4">
          ...
          <p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-muted-foreground text-right leading-tight">{label}</p>
        </div>
        <p className={cn("text-[1.875rem] font-bold tracking-tight leading-none",
...
function DonutLabel({ cx, cy, total }: { cx: number; cy: number; total: number }) {
  return (
    <>
      <text x={cx} y={cy - 8} textAnchor="middle" className="fill-foreground" style={{ fontSize: 20, fontWeight: 700 }}>
        {formatBRL(total)}
      </text>
    ...
    <div className="min-h-svh bg-background text-foreground">
      ...
      <main className="max-w-7xl mx-auto px-6 py-5 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          />
          <KpiCard
            label={isPncp ? "Contratos PNCP" : "Contratos demonstrativos"}
            value={totalContratos.toString()}
            sub={periodoLabel(contratosPorMes)}
            icon={FileText}
          />
          <KpiCard
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={contratosPorMes} barSize={14} margin={{ top: 4, right: 8, bottom: 0, left: 8 }}>
                <XAxis dataKey="mes" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 10 }} axisLine={false} tickLine={false} />
                <YAxis tickFormatter={(v) => formatBRL(v)} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 10 }} axisLine={false} tickLine={false} width={68} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription>Distribuição demonstrativa do orçamento 2025</CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={despesaSecretaria} dataKey="valor" nameKey="secretaria" cx="50%" cy="44%" innerRadius={68} outerRadius={100} paddingAngle={2}>
                    {despesaSecretaria.map((entry, i) => (<Cell key={i} fill={entry.cor} />))}
                    <DonutLabel cx={160} cy={114} total={MUNICIPIO.orcamento_2025} />
                  </Pie>
                  <Legend iconType="circle" iconSize={7} formatter={(value) => (<span style={{ fontSize: 11, color: "hsl(var(--muted-foreground))" }}>{value}</span>)} />
                  <Tooltip formatter={(value) => [formatBRLFull(Number(value)), ""]} contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 10, fontSize: 12 }} />
                </PieChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription>Principais fontes demonstrativas de arrecadação 2025</CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={receitaOrigem} dataKey="valor" nameKey="categoria" cx="50%" cy="44%" innerRadius={68} outerRadius={100} paddingAngle={2}>
                    {receitaOrigem.map((entry, i) => (<Cell key={i} fill={entry.cor} />))}
            ...
            <div className="flex flex-col gap-4">
              {topEmpresas.map((e, i) => {
                ...
                return (
                  <div key={i}>
                    <div className="flex items-center gap-3">
                      <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center text-[11px] font-bold shrink-0 ring-1", palette)}>
                        {getInitials(e.empresa)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-3">
                          <span className="text-sm font-medium truncate">{e.empresa}</span>
                          <span className="text-sm font-semibold text-primary shrink-0">{formatBRL(e.valor)}</span>
                        </div>
                        <div className="flex items-center justify-between gap-3 mt-0.5">
                          <span className="text-[10px] text-muted-foreground font-mono">{e.cnpj}</span>
                          <span className="text-xs text-muted-foreground shrink-0 hidden sm:block">{e.contratos} contratos</span>
                        </div>
                      </div>
                    </div>
                    <div className="mt-2 ml-11 h-1 rounded-full bg-muted overflow-hidden">
                      <div className={cn("h-full rounded-full transition-all", i === 0 ? "bg-primary" : "bg-primary/35")} style={{ width: `${pct}%` }} />
                    </div>
                  </div>
                );
              })}
          ...
          <CardContent>
            <div className="flex flex-col">
              {ultimosContratos.map((c, i) => (
                <div key={i} className={cn("flex items-start gap-3 py-3.5", i < ultimosContratos.length - 1 && "border-b border-border/40")}>
                  <div className={cn("w-1.5 h-1.5 rounded-full mt-[7px] shrink-0", secretariaDot(c.secretaria))} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                          <span className="text-[10px] font-mono text-muted-foreground/60">{c.numero}</span>
                          <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-4">{c.secretaria}</Badge>
                          <Badge variant="secondary" className="text-[10px] px-1.5 py-0 h-4">{c.modalidade}</Badge>
                        </div>
                      </div>
        ...
        <div className="flex items-start gap-2.5 text-xs text-muted-foreground/70 pb-4 pt-1">
          {isPncp ? (
            <>
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400/80 shrink-0 mt-px" />
              <span>
                <strong className="text-muted-foreground font-medium">Dados reais PNCP</strong> — contratos
                de Americana/SP (IBGE{" "}
                <code className="text-[10px] bg-muted px-1 py-0.5 rounded font-mono">3501608</code>).
                {coletadoEm && (
            ...
            <>
              ...
              <span>
                ...
                baseada no PNCP ({" "}
                <code className="text-[10px] bg-muted px-1 py-0.5 rounded font-mono">codigoMunicipio=3501608</code>
